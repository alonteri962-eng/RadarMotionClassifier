% =========================================================================
% FMCW Radar Signal Processing + ML Target Classification (Software-Only)
%
% What this script demonstrates (end-to-end):
%   1) FMCW waveform generation (77 GHz automotive-style radar parameters)
%   2) Multi-target received-signal simulation (delay + Doppler per target)
%   3) De-chirp mixing to obtain beat signal
%   4) Range FFT (fast-time) and Doppler FFT (slow-time) → Range–Doppler Map
%   5) Micro-Doppler-style time-frequency view at a selected range bin
%   6) Synthetic ML classification (Human / Car / Drone) using velocity
%      profiles and simple engineered features
%
% Toolboxes:
%   - Base MATLAB
%   - Recommended: Signal Processing Toolbox (spectrogram)
%   - Recommended: Statistics & Machine Learning Toolbox (fitcecoc)
% =========================================================================

clear; close all; clc;

%% ------------------------ 1. Radar Parameters ---------------------------
% This section defines the physical constants and the FMCW radar parameters.
% The chosen carrier (77 GHz) and bandwidth (150 MHz) are typical for
% automotive radar demonstrations.
%
% Key concepts:
%   - Fast-time: samples within a single chirp (range information)
%   - Slow-time: chirp-to-chirp progression (Doppler/velocity information)
...
%% ------------------------ 2. Target Definition --------------------------
% Define a simple multi-target scene.
% Each row: [range (m), radial velocity (m/s), amplitude]
%
% Interpretation:
%   - Range sets the propagation delay (tau = 2R/c)
%   - Velocity sets Doppler (fd = 2v/lambda)
%   - Amplitude scales the reflected signal strength
...
% Build the transmitted FMCW frame as a 2D matrix:
%   rows    → chirp index (slow-time)
%   columns → samples within chirp (fast-time)
%
% The transmitted chirp is modeled as a complex exponential with:
%   phase = 2π( f_c*t + 0.5*slope*t^2 )
...
% Simulate the received signal as a sum of reflections from all targets.
% For each target:
%   - Apply a constant round-trip delay tau based on its range
%   - Apply Doppler shift fd based on its radial velocity
%
% Note:
%   This is a simplified narrowband reflection model used for educational
%   demonstration of range/Doppler processing.
...
% This emulates a more realistic measurement environment.
SNR_dB = 20;
rx = awgn(rx, SNR_dB, 'measured');

%% ------------------------ 5. De-chirp (Mixing) --------------------------
% De-chirping (a.k.a. deramping) mixes the received signal with the
% conjugate of the transmitted chirp.
%
% Result:
%   The output "mix" contains beat frequencies that encode target range,
%   while phase evolution across chirps encodes Doppler/velocity.

mix = tx .* conj(rx);

%% ------------------------ 6. Range FFT (fast-time) ----------------------
% Range estimation is performed by an FFT across fast-time (within each chirp).
% Using zero-padding (power-of-two FFT length) improves interpolation in the
% frequency domain.
%
% Only the non-negative frequencies are retained because they correspond to
% physically meaningful beat frequencies in this simplified model.
...
% Doppler estimation is performed via FFT across slow-time (across chirps).
% fftshift is used to center zero Doppler.
%
% PRF here is approximated as 1/Tchirp (chirp repetition frequency).
% Doppler is mapped to velocity using v = (fd * lambda)/2.
...
% Plot the Range–Doppler Map (RDM) in dB scale.
% This visualization is the standard 2D output of FMCW processing chains,
% highlighting targets by their range and radial velocity.
...
% Choose the strongest range bin (energy-based selection) and inspect its
% Doppler behavior over slow-time.
%
% A spectrogram (STFT) provides a time-frequency view that can reveal
% micro-motion patterns (micro-Doppler-like signatures).
...
%
% Motivation:
%   Running full FMCW simulation for every ML example is computationally heavy.
%   Instead, we simulate class-dependent velocity profiles over slow-time and
%   derive simple features from them.
%
% Approach:
%   1) Generate synthetic velocity profiles per class
%   2) Convert velocity to Doppler (fd = 2v/lambda)
%   3) Build a simple phase-accumulated Doppler signal
%   4) Extract interpretable features from velocity
%   5) Train a multi-class classifier and evaluate accuracy
% ========================================================================

%% ------------------------ 10. Class Setup -------------------------------
% Define target classes and dataset size.
% Each class contributes Nexamples_per_class samples for supervised learning.
...
% Create a wrapper handle for a local function that generates velocity
% patterns per class. This keeps the main loop readable.
...
for ci = 1:Nc
    ...
    for n = 1:Nexamples_per_class
        ...
        % Simple, interpretable statistics that often work well as a baseline.
...
% rng(1) ensures the same split every run (reproducibility).
...
% Train a multi-class SVM classifier using ECOC (Error-Correcting Output Codes).
% This is a common approach for extending binary SVMs to multi-class tasks.
...
% Plot a 2D projection of the feature space to understand class separability.
% Here we visualize Mean velocity vs Max velocity for intuition.
...
% Local helper function for generating synthetic velocity profiles.
% MATLAB requires local functions to appear at the end of a script.
